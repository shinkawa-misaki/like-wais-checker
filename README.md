...
WAIS（ウェクスラー成人知能検査）を参考に設計した、**4つの認知指数の傾向を測るセルフチェックツール**です。
...
> **免責事項**：このツールはWAIS標準化検査ではありません。年齢別ノームがないためIQは算出できません。結果は自己理解・戦略設計の参考指標として使用してください。
...
`wais_mysql` が `healthy` になるまで待ってから再実行してください。
